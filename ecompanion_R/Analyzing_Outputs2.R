...
list_of_files_CC = list.files(path = "./Results2gamma0lessgammak/", recursive = TRUE, pattern = "^CC_.*\\.csv$", full.names = TRUE)
datalist_CC = lapply(list_of_files_CC, read.csv)

list_of_files_MNL = list.files(path = "./Results2gamma0lessgammak/", recursive = TRUE, pattern = "^MNL_.*\\.csv$", full.names = TRUE)
...
write.csv(output_table, file = "summary_table2.csv", row.names = FALSE)
